...
def conv_forward_naive(x, w, b, conv_param):
	...
	H_new = 1 + (H - HH + 2*pad) // stride
	W_new = 1 + (W - WW + 2*pad) // stride
	s = stride
		...
		for j in range(F):
			for k in range(H_new):
				for l in range(W_new):
					out[i,j,k,l] = np.sum(x_padded[i,:, k*s:HH+k*s, l*s:WW+l*s]*w[j]) + b[j]
	#############################################################################
	#                             END OF YOUR CODE                              #
	#############################################################################
	cache = (x, w, b, conv_param)
...
def conv_backward_naive(dout, cache):
	...
	for i in range(N):
		for j in range(F):
			for k in range(H_new):
				for l in range(W_new):
					window = x_padded[i, :, k*s:HH+k*s, l*s:WW+l*s]
					db[j] += dout[i,j,k,l]
					dw[j] += window*dout[i,j,k,l]
					dx_padded[i, :, k*s:HH+k*s, l*s:WW+l*s] += w[j] * dout[i,j,k,l]
